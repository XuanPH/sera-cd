...
class o2oApi {
    ...
    try {
      ...
      if (err.response && err.response.status === 401) {
        throw {
          code: "unAuthorize",
          msg:
            "We can't connect to O2O CRM. Please update your O2O Token in <b>Admin > Manage > SERA-CD > Change Setting </b>"
        };
        ...
        throw {
          code: "connectFailed",
          msg: "We can't connect to server please check your network"
        };
      }
    }
  }
    ...
    if (zenUser.id) {
      try {
        ...
        if (Object.keys(data).length === 0) {
          throw {
            code: "leadNotFound",
            msg: "We can't find this people on O2O CRM!"
          };
        }
  ...
  async _getDetailHistoryWebAccess(page, pageSize) {
    try {
      const res = await axios.get(
        `${BASE_URL}/lead/weblog?leadId=${this.leadId}&page=${page}&pageSize=${pageSize}`,
        this.config
      );
      return res;
    } catch (err) {
  ...
  async _getLeadActivities(lastId = 0, filterType = ["all"]) {
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/lead/activity?leadId=${this.leadId}`,
        body,
        this.config
      );
      return res;
  ...
  async _referenceData(type) {
    ...
    try {
      const res = await axios.post(
        `${BASE_URL}/ReferenceData/GetByQuery`,
        body,
        this.config
      );
      return res;
...
export function filterItem() {
  ...
  if (existsFilterLocal) return existsFilterLocal;
  //init when open app
...
export function refDataAjax(type) {
  return {
    url: "",
    headers: {
