...
import { _initModal } from "../../templates/customer_info";
import "select2";
...
class Modal {
  constructor(client, appData) {
    ...
    // construct o2oApi
    var trigger_modal_data = getLocalStorage("trigger_modal_data");
    try {
      this.passingDataContruct(trigger_modal_data);
    } catch (err) {
    ...
    //   this.dataUser = template_getting_type.dataUser ?
    //     template_getting_type.dataUser : {};
    //   this._content = template_getting_type.content ?
    //     template_getting_type.content :
    //     "";
    //   this._parentClient = client.instance(template_getting_type.parentGuid);
    //   this.o2oApi = new o2oApi(
    //     template_getting_type.o2oApi.token,
    //     template_getting_type.o2oApi.leadId
    //   );
    //   this.webAccessedModal = new WebAccessDetailModel(this.o2oApi);
    //   this.webAccessChart = new WebAccessChart(
    //     template_getting_type.type_chart
    //   );
    ...
    setTimeout(() => {
      if (!this.o2oApi) {
        this._content =
          "We cant bind this form to zendesk, please refresh that page";
  ...
  passingDataContruct(trigger_modal_data) {
    this._parentClient = this._client.instance(trigger_modal_data.parentGuid);
    this.dataUser = trigger_modal_data.dataUser;
    this._content = trigger_modal_data.content
      ? trigger_modal_data.content
      : "";
    this.o2oApi = new o2oApi(
      trigger_modal_data.o2oApi.token,
      trigger_modal_data.o2oApi.leadId
    );
    this.webAccessedModal = new WebAccessDetailModel(this.o2oApi);
    this.webAccessChart = new WebAccessChart(trigger_modal_data.type_chart);
    this.customerUpdate = new CustomerUpdate(
      this._client,
      this.dataUser,
      this.o2oApi,
    ...
    this.initializePromise = this.init(trigger_modal_data.type);
    localStorage.removeItem("trigger_modal_data");
  }
}
